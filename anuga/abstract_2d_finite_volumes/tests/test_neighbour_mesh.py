...
class Test_Mesh(unittest.TestCase):
    ...
    def test_reorder(self):
        ...
        new_neighbours = new_mesh.neighbours
        new_neighbour_edges = new_mesh.neighbour_edges
        new_number_of_boundaries = new_mesh.number_of_boundaries
        new_surrogate_neighbours = new_mesh.surrogate_neighbours
        new_vertex_coordinates = new_mesh.vertex_coordinates
        new_edge_midpoint_coordinates = new_mesh.edge_midpoint_coordinates
        new_normals = new_mesh.normals
        new_areas = new_mesh.areas
        new_edgelengths = new_mesh.edgelengths
        new_centroid_coordinates = new_mesh.centroid_coordinates
        new_radii = new_mesh.radii

        new_number_of_triangles_per_node = new_mesh.number_of_triangles_per_node
        new_vertex_value_indices = new_mesh.number_of_triangles_per_node
        new_tagged_elements = new_mesh.tagged_elements

        # from original mesh create a reordered mesh
        import copy

        reorder_mesh = copy.deepcopy(mesh)
        reorder_mesh.reorder(new_order, in_place = True)

        reorder_nodes = reorder_mesh.get_nodes()
        reorder_triangles = reorder_mesh.get_triangles()
        reorder_boundary = reorder_mesh.boundary
        reorder_neighbours = reorder_mesh.neighbours
        reorder_neighbour_edges = reorder_mesh.neighbour_edges
        reorder_number_of_boundaries = reorder_mesh.number_of_boundaries
        reorder_surrogate_neighbours = reorder_mesh.surrogate_neighbours
        reorder_vertex_coordinates = reorder_mesh.vertex_coordinates
        reorder_edge_midpoint_coordinates = reorder_mesh.edge_midpoint_coordinates
        reorder_normals = reorder_mesh.normals
        reorder_areas = reorder_mesh.areas
        reorder_edgelengths = reorder_mesh.edgelengths
        reorder_centroid_coordinates = reorder_mesh.centroid_coordinates
        reorder_radii = reorder_mesh.radii

        reorder_number_of_triangles_per_node = reorder_mesh.number_of_triangles_per_node
        reorder_vertex_value_indices = reorder_mesh.number_of_triangles_per_node
        reorder_tagged_elements = reorder_mesh.tagged_elements

        # test that reorder is consistent
        num.testing.assert_allclose(new_nodes, reorder_nodes)
        num.testing.assert_allclose(new_triangles, reorder_triangles)
        self.assertDictEqual(new_boundary, reorder_boundary)

        # FIXME SR: seems the numbering of the boundary can change.
        #mask = new_neighbours >= 0
        #num.testing.assert_allclose(new_neighbours[mask], reorder_neighbours[mask])
        num.testing.assert_allclose(new_neighbours, reorder_neighbours)
        ...
        self.assertEqual(new_tagged_elements.keys(), reorder_tagged_elements.keys())
        ...
        num.testing.assert_allclose(new_vertex_coordinates, reorder_vertex_coordinates)
        num.testing.assert_allclose(new_edge_midpoint_coordinates, reorder_edge_midpoint_coordinates)
        num.testing.assert_allclose(new_normals, reorder_normals)
        num.testing.assert_allclose(new_areas, reorder_areas)
        num.testing.assert_allclose(new_edgelengths, reorder_edgelengths)
        num.testing.assert_allclose(new_centroid_coordinates, reorder_centroid_coordinates)
        num.testing.assert_allclose(new_radii, reorder_radii)
