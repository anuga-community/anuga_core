...
import re
import numpy as np
import linecache  # still used for elevation load at startup
import itertools
import datetime
# -----------------------------------------------------------------------------
...
#profiler.enable()
# ------------------------------------------------------------------------------
# Rainfall: preload every yieldstep's file ONCE (rank 0 reads, broadcast),
# no file I/O inside the evolve loop.  See rain_cache.py.
# ------------------------------------------------------------------------------
from rain_cache import CachedRain
_D = Current_Date.strftime(TIMEFORMATCU)
def _p(sub, prefix):
    return lambda t: f"rainfall_data/{sub}/{_D}/{prefix}_{_D}_{str(t).replace('.', '_')}.csv"
rain_sources = [   # same priority order as the original if/elif chain
...
tsub0 = time.time()
cached_rain = CachedRain(domain, yieldstep, finaltime, daily_time, rain_sources, verbose=verbose)
barrier()
if myid == 0: print('Rain preload: Time', time.time() - tsub0, flush=True)

for t in domain.evolve(yieldstep=yieldstep, finaltime=finaltime):
    if myid == 0: domain.write_time()
    cached_rain.apply(t, Q, rain_operator)
