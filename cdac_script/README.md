...
Loads the prepartitioned data from `partitions/` and gets going asap.

### Configuration

Edit the script to set mesh size:
```python
MESH_SIZE = 900  # <-- CHANGE THIS for different mesh sizes
```

### Running with rainfall/tide/discharge data

**IMPORTANT**: Pass the date argument to load actual data files:
```bash
python run_model_3_partitioned.py 10-09-2021
```

Without the date argument, it defaults to today's date which won't find data files.

### Performance tuning

The script has a `STATS_INTERVAL` parameter (default 360) that controls how often expensive MPI reductions are performed for volume/statistics. With yieldstep=5:
- `STATS_INTERVAL = 360` → stats every 30 min of simulation time
- `STATS_INTERVAL = 720` → stats every 1 hour
- `STATS_INTERVAL = 2160` → stats every 3 hours

Lower values = more frequent stats but more MPI overhead.

## rainfall_loader.py

Pre-loads all rainfall CSV files at startup instead of reading line-by-line during simulation. This eliminates the slow `linecache.getline()` calls.

Used automatically by `run_model_3_partitioned.py`. 
