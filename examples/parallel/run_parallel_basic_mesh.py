...
#
#  The key difference from run_parallel_rectangular.py is that
#  only rank 0 builds the mesh (as a lightweight Basic_mesh),
#  and quantities are set per-rank after distribution rather
#  than being copied from a full Domain on rank 0.
#
#  To run in parallel on 4 processes:
#
#    mpiexec -np 4 python -u run_parallel_basic_mesh.py
#
#########################################################
...
parser = argparse.ArgumentParser(description='Rectangular domain via distribute_basic_mesh')
...
# ---------------------------------------------------------------------------
# Step 2: Distribute mesh to all ranks
# ---------------------------------------------------------------------------
barrier()
if myid == 0:
...
domain = distribute_basic_mesh(bm, verbose=verbose, parameters=dist_params)
domain.set_name('sw_rectangle_basic_mesh')
